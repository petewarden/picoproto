...
#define PP_EXPECT(X) if (!(X)) { PP_LOG(ERROR) << #X \
<< " expected to be true, but was " << (X) << std::endl; return false; }

#define PP_EXPECT_EQ(X, Y) if ((X) != (Y)) { PP_LOG(ERROR) << #Y \
  << " expected to be " << #X << ", but was " << (Y) << std::endl; return false; }

#define PP_EXPECT_NE(X, Y) if ((X) == (Y)) { PP_LOG(ERROR) << #Y \
<< " expected not to be " << #X << ", but was" << std::endl; return false; }

#define PP_EXPECT_NEAR(X, Y, E) { double delta = ((X) - (Y)); \
if (delta < 0) { delta = - delta; } \
if (delta > (E)) { PP_LOG(ERROR) << #Y \
<< " expected to be near " << #X << ", but was " << (Y) << std::endl;  \
return false; } }
...
TestFunction("String", []{
  std::vector<uint8_t> bytes({0x12, 0x07, 0x74, 0x65, 0x73, 0x74, 0x69, 0x6e, 0x67});
  Message message;
  message.ParseFromBytes(bytes.data(), bytes.size());
...
TestFunction("UInt64", []{
  std::vector<uint8_t> bytes({0x09, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00});
  Message message;
  message.ParseFromBytes(bytes.data(), bytes.size());
